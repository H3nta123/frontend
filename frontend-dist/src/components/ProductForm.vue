  ...
  <v-container fluid class="pa-0 mw-1000">
    ...
    <div class="d-flex align-center justify-space-between mb-6">
      <div class="d-flex align-center">
        <v-btn v-if="!isIframe" icon="mdi-arrow-left" variant="text" to="/catalog" class="mr-2"></v-btn>
        <h1 class="text-h5 font-weight-bold">{{ isEditMode ? 'Редактировать товар' : 'Добавить товар' }}</h1>
      </div>
      <div class="d-flex gap-2">
        <v-btn v-if="!isIframe" variant="outlined" color="black" class="text-none font-weight-bold" rounded="lg" to="/catalog">
          Отмена
        </v-btn>
        <v-btn 
          color="#37E23F" 
...
const isEditMode = computed(() => {
  return (route.params as any).id && (route.params as any).id !== 'new'
})

const productId = computed(() => {
  return isEditMode.value ? Number((route.params as any).id) : null
})
...
const handleSave = async () => {
  ...
  if (success) {
    // В идеале возвращаться назад или показывать уведомление
    if (isIframe.value) {
        // Если в iframe, может просто сбросить/показать успех?
        // Но пока оставим редирект, который просто обновит iframe
        router.push('/catalog') 
    } else {
        router.push('/catalog')
    }
